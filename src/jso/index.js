...
export const jso_getAuthRequest = (providerid, scopes) => {
...
*/
const jso_authrequest = (providerid, scopes) => {
  let state, request, authurl;

  authurl = jso_getAuthUrl(providerid, scopes);
...
export const generatePKCE = () => {
  ...
  const codeVerifierBytes = window.crypto.getRandomValues(array);
  const codeVerifier = base64URLEncode(window.crypto.getRandomValues(array));
  const codeChallenge = base64URLEncode(
    crypto.subtle.digest("SHA-256", codeVerifierBytes)
  );
